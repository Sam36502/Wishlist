...
func GetAllUsers() ([]*User, error) {
	...
	if err != nil {
		return nil, inf.StackError(err, "Failed to get all users:")
	}
	defer rows.Close()
...
func GetUserWithEmail(email string) (*User, error) {
	...
	if err != nil {
		return nil, inf.StackError(err, "Failed to read user from DB")
	}
...
func InsertUser(user *User) (*User, error) {
	...
	if err == nil {
		err = inf.EmailExistsError(fmt.Sprintf("Email '%s' already registered.", user.Email))
		return nil, inf.StackError(err, "Failed to insert new user")
	}
	...
	if err != nil {
		return nil, inf.StackError(err, "Failed to retrieve newly created user")
	}
...
func (usr *User) ChangeEmail(new_email string) error {
	...
	if err == nil {
		return inf.EmailExistsError(fmt.Sprintf("Email '%s' already registered.", usr.Email))
	}
	...
	if err != nil || newuser.Email != new_email {
		return inf.StackError(err, "Failed to confirm user was updated:")
	}
...
func (usr *User) ChangePassword(new_pass string) error {
	...
	if err != nil || newuser.Password != phash {
		return inf.StackError(err, "Failed to retrieve newly created user:")
	}
